...
class TweetsListViewModel: TweetsListViewModelType {
    ...
    private let twitterUser: Variable<User>
    ...
    var cellViewModels: Observable<[TweetsListSectionViewModel]> {
        return tweets.asObservable().map {
            ...
            let section = TweetsListSectionViewModel(model: LocalizedConstants.Tweets.tweetsSectionMessage, items: vms)
            return [section]
        }
    }
    ...
    init(twitterUser: User, twitterDataSource: TwitterDataSourceType) {
